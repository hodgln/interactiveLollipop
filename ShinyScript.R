...
geneSymbols <- data.frame((read.csv("/Users/danhodgin/Documents/University/Intercalation/annotation/mutationsEdit.csv"))["Hugo_Symbol"])

listGenes <- unique(geneSymbols)

# change the file path to your own
mutation.csv <- "/Users/danhodgin/Documents/University/Intercalation/annotation/mutationsEdit.csv"
...
shinyApp(
  ...
  server = function(input, output) {
    ...
    # try if the gene selected is null
    
    #   if (!is.null(reactive(input$gene))) {
    #   newGene <- reactive(input$gene)
    # } else {
    #   newGene <- reactive("Hello")
    # }
    ...
    #plot.options.g3Lollipop.options.y.axis.label <- paste("# of ", geneSelected(), "gene mutations")
